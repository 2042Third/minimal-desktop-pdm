<script lang="ts">
    ...
    try {
      ...
      if ($results.error) {
        notifications.add({
          type: 'error',
          title: 'Error',
          message: $results.error || 'Something went wrong',
          actions: [
            {
              label: 'Retry',
    ...
    try {
      ...
      if (output.error) {
        notifications.add({
          type: 'error',
          title: 'Error',
          message: output.error || 'Something went wrong',
          actions: [
            {
              label: 'Retry',
        ...
        notifications.add({
          type: 'success',
          title: 'Statement executed successfully',
          message: `Last Insert ID (${output.lastInsertId}); Rows affected (${output.rowsAffected})`,
          duration: 5000
        });
      }
    } catch (error: any) {
  ...
  async function loadTables() {
    try {
      const result: QueryResult = await Database.ExecuteQuery(
